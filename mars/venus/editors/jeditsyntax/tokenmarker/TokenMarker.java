...
public abstract class TokenMarker {
	...
	public Token markTokens(Segment line, int lineIndex) {
		 ...
		 * inserted into the text, firing a document event, the insertUpdate() method
		 * shifts the caret (if necessary) by the amount inserted.
		 * 
		 * All caret movement is handled by the select() method, which eventually pipes
		 * the new position to scrollTo() and calls repaint().
		 * 
		 * Note that at this point in time, the new line hasn't yet been painted; the
		 * caret is moved first.
		 * 
		 ...
		 * 
		 * After scrollTo() has done it's job, repaint() is called, and eventually we
		 * end up in paintLine(), whose job is to paint the changed line. It, too, calls
		 * markTokens().
		 * 
		 * The problem was that if the line started a multiline token, the first
		 * markTokens() (done in offsetToX()) would set nextLineRequested (because the
		 * line end token had changed) but the second would clear it (because the line
		 * was the same that time) and therefore paintLine() would never know that it
		 * needed to repaint subsequent lines.
		 * 
		 * This bug took me ages to track down, that's why I wrote all the relevant info
		 * down so that others wouldn't duplicate it.
		 */
