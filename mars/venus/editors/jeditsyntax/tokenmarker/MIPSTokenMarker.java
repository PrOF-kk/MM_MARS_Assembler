...
public class MIPSTokenMarker extends TokenMarker {
				...
				switch (c) {
					...
					case ':':
						...
						*/
						...
						try {
							validIdentifier = mars.assembler.TokenTypes
	...
	public ArrayList<PopupHelpItem> getTokenPrefixMatchHelp(String line, Token tokenList, Token token, String tokenText) {
		...
		// CASE: Current token is valid KEYWORD1 (MIPS instruction). If this line contains
		//       a previous KEYWORD1 or KEYWORD2 token, then we ignore this one and do exact match on the first one.
		//       If it does not, there may be longer instructions for which this is a prefix,
		//       so do a prefix match on current token.
		if (token != null && token.id == Token.KEYWORD1) {
		...
		//       something other than KEYWORD1 or KEYWORD2.
		//       Generate text based on exact match of that token.
		if (keywordTokenText != null) {
		...
		//       as being either white space or an in-progress token possibly preceded by white space.
		//       We'll do a trim on the token. Now there are two subcases to consider:
		//    SUBCASE: The line does not contain any KEYWORD1 or KEYWORD2 tokens but
		//             nothing remains after trimming the current token's text.
		//             This means it consists only of white space and there is
		//             nothing more to do but return.
		//    SUBCASE: The line does not contain any KEYWORD1 or KEYWORD2 tokens. This means
		//             we do a prefix match of the current token to either instruction or directive names.
		//             Easy to distinguish since directives start with "."
	...
	public static KeywordMap getKeywords() {
		if (cKeywords == null) {
			...
			// Note: Coprocessor 0 registers referenced only by number: $8, $12, $13, $14.
			// These are already in the map
