...
public class JEditTextArea extends JComponent 
{
	...
	public JEditTextArea(TextAreaDefaults defaults, JComponent lineNumbers)
	{
		...
		KeyboardFocusManager.getCurrentKeyboardFocusManager().addKeyEventDispatcher( 
				e -> {
					if (JEditTextArea.this.isFocusOwner()
		...
		try
		{
			if(rectSelect)
			{
				...
				for(int i = selectionStartLine; i <= selectionEndLine; i++)
				{
					...
					document.remove(rectStart,
							Math.min(lineEnd - rectStart, end - start));
				...
				if(selectedText != null && currNewline != selectedText.length())
				{
					...
					document.insertString(offset + 1,
							selectedText.substring(currNewline + 1),null);
				}
			}
			else
			{
	 ...
	 */
	// Used for tool tip only (not popup menu)
	...
	private ArrayList<PopupHelpItem> getSyntaxSensitiveHelpAtLineOffset(int line, int offset, boolean exact) {
		...
		if (tokenAtOffset != null) {
			...
			if (exact) {
				...
				matches = tokenMarker.getTokenPrefixMatchHelp(lineSegment.toString(),
						tokenList, tokenAtOffset, tokenText);
			}
		}
		return matches;
	}
	...
	private void applySyntaxSensitiveHelp() {
		...
		int line = getCaretLine();
		int lineStart = getLineStartOffset(line);
		int offset = Math.max(1, Math.min(getLineLength(line), getCaretPosition() - lineStart));
			...
			if (item.isEnabled()) {
				...
				if (listeners.length > 0) {
					listeners[0].actionPerformed(
							new ActionEvent(item, ActionEvent.ACTION_FIRST,
